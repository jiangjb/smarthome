var locat = (window.location+'').split('/');    //路径没问题
...
//	alert("locat:"+locat);//http://localhost:8080/smarthomeMavenWebProject
	//alert("locat+MENU_URL:"+locat+MENU_URL);//http://localhost:8080/smarthomeMavenWebProject/login.do
	top.mainFrame.tabAddHandler(id,MENU_NAME,locat+MENU_URL);//MENU_URL要处理
	if(MENU_URL != "druid/index.html"){//这里要替换
		jzts();
	}
}*/
...
function goOut(msg){
	alert(msg);
	window.location.href=locat+"/logout.do";
}
//强制某用户下线
